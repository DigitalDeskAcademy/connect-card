...
async function main() {
  ...
  const churchOwner = await prisma.user.upsert({
    ...
    create: {
      ...
      organizationId: newlifeOrg.id,
      emailVerified: true,
      defaultLocationId: bainbridgeLocation.id, // Default location for uploads
      createdAt: new Date(),
      updatedAt: new Date(),
    },
  });
  ...
  // ========================================
  const bainbridgeBatch = await prisma.connectCardBatch.create({
    data: {
      organizationId: newlifeOrg.id,
      locationId: bainbridgeLocation.id,
      uploadedBy: churchOwner.id,
      name: `Bainbridge - Nov 17, 2025`,
      status: "PENDING",
      cardCount: 0,
    },
  });

  const bremertonBatch = await prisma.connectCardBatch.create({
    data: {
      organizationId: newlifeOrg.id,
      locationId: bremertonLocation.id,
      uploadedBy: churchOwner.id,
      name: `Bremerton - Nov 17, 2025`,
      status: "PENDING",
      cardCount: 0,
    },
  });
  ...
  for (let i = 0; i < awaitingReviewCards.length; i++) {
    ...
    await prisma.connectCard.create({
      data: {
        ...
        batchId: i < 3 ? bainbridgeBatch.id : bremertonBatch.id,
  ...
  for (let i = 0; i < reviewedCards.length; i++) {
    ...
    await prisma.connectCard.create({
      data: {
        ...
        batchId: i % 2 === 0 ? bainbridgeBatch.id : bremertonBatch.id,
  ...
  for (let i = 0; i < volunteerOnboardingCards.length; i++) {
    ...
    await prisma.connectCard.create({
      data: {
        ...
        batchId: i === 0 ? bainbridgeBatch.id : bremertonBatch.id,
